...
export const GooglePay = ({cart}: IProps) => {
    ...
    const paymentMethods = toArray(process.env.NEXT_PUBLIC_GP_CARD_NETWORKS) as google.payments.api.CardNetwork[]
    const authMethods = toArray(process.env.NEXT_PUBLIC_GP_AUTH_METHODS) as google.payments.api.CardAuthMethod[]
    ...
    const allowedPaymentMethods = [{
        ...
        tokenizationSpecification: {
            ...
            parameters: {
                ...
                gatewayMerchantId: process.env.NEXT_PUBLIC_GP_GATEWAY_MERCHANT_ID ?? "",
            },
        },
    ...
    const getShippingPrices = (option: ShippingOptions): string => {
        switch(option) {
            case 'shipping-001': return '0.00'
            case 'shipping-002': return '1.99'
            case 'shipping-003': return '10.00'
        }
    }

    const calculateTransactionInfo = (shippingId: ShippingOptions): google.payments.api.TransactionInfo => {
        let totalPrice = 0;

        const shippingDetails: google.payments.api.DisplayItem[] = cart.products.map(p => {
            totalPrice += p.price;
    ...
    return(
        <GooglePayButton
            ...
            onPaymentDataChanged={(ipd) => new Promise((resolve, reject) => {
                ...
                if (ipd.callbackTrigger == "INITIALIZE" || ipd.callbackTrigger == "SHIPPING_ADDRESS") {
                    intermediaryPaymentRequest.newShippingOptionParameters = allShippingOptions;
                    let selectedShippingOptionId = intermediaryPaymentRequest.newShippingOptionParameters.defaultSelectedOptionId;
                    intermediaryPaymentRequest.newTransactionInfo = calculateTransactionInfo(selectedShippingOptionId as ShippingOptions);
                }
                else if (ipd.callbackTrigger == "SHIPPING_OPTION" && shippingOptions) {
                    intermediaryPaymentRequest.newTransactionInfo = calculateTransactionInfo(shippingOptions.id as ShippingOptions);
                }
