...
const SideBar = (props:any) => {
    ...
    const checkPath = (path:string) => {
      return path==='/profile' ? (router.pathname === '/profile' ? true : false)
                                : ((router.pathname.includes(path)) ? true : false);
    }
    const profileArray = [
        {
        ...
        <div className="overflow-y-auto py-4 px-3">
            ...
            </li>
            {
                profileArray.map((ele, index)=>{
                    ...
                    return (
                        <li key={index}>
                            <Link href={ele.path} className="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                                <span className={(flag?"text-[#A042E1] ":"") +"flex-1 whitespace-nowrap"}>{ele.name}</span>
                                <svg className={flag?"inline-block":"hidden"} xmlns="http://www.w3.org/2000/svg" width="10" height="11" shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 298 511.93"><path fillRule="nonzero" d="M70.77 499.85c-16.24 16.17-42.53 16.09-58.69-.15-16.17-16.25-16.09-42.54.15-58.7l185.5-185.03L12.23 70.93c-16.24-16.16-16.32-42.45-.15-58.7 16.16-16.24 42.45-16.32 58.69-.15l215.15 214.61c16.17 16.25 16.09 42.54-.15 58.7l-215 214.46z"/></svg>
                            </Link>
                        </li>
                    );
                })
