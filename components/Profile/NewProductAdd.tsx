  ...
  const handleUploadClick = (e:any) => {
    ...
    [...e.target.files].forEach((file) => {
        ...
        reader.onload = function (evt:any) {
            ...
            if (temp.length === e.target.files.length) setFiles([...files, ...temp]);
        };
    ...
    <div className='p-16 w-full'>
      ...
      <div className='max-w-[900px] font-comfortaa'>
        ...
        <div className='max-w-[300px] mt-12'>
          ...
          <input type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 pl-8" placeholder="Add numerical value" required />
        </div>
        <div className='mt-12'>
          <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Product Image</label>
          <span className='text-gray-400'>Use .png or .jpg format, max 7 photos, 25mb per photo</span>
          <div className='flex mt-2'>
            {
              files.map((ele,index)=>{
                return <img src={ele.data} key={index} className="w-[60px] h-[60px] mx-2 rounded-lg"/>;
              })
            }
            <label htmlFor="contained-button-file" className='w-[60px] h-[60px] rounded-lg border-2 border-gray-300 border-dashed flex items-center justify-center cursor-pointer'>
              <input
                  accept=".png,.jpg"
        ...
        <div className='mt-12 flex'>
          ...
          <div className='max-w-[300px] w-full ml-5'>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Product Subcategory</label>
            <select className="w-full p-2.5 text-gray-500 bg-white border rounded-md shadow-sm outline-none appearance-none focus:border-gray-600">
        ...
        <div className='mt-12 flex'>
          ...
          <div className='max-w-[300px] w-full ml-5'>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ships from</label>
            <select className="w-full p-2.5 text-gray-500 bg-white border rounded-md shadow-sm outline-none appearance-none focus:border-gray-600">
              <option>USA</option>
              <option>Italy</option>
            </select>
        ...
        <div className='mt-12'>
          <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Product Filters</label>
          <span className='text-gray-400'>Use .png or .jpg format, max 7 photos, 25mb per photo</span>
          <div className='flex mt-2'>
            <Menu as="div" className="relative inline-block text-left w-[60px] h-[60px] rounded-lg border-2 border-gray-300 border-dashed ">
              <div>
          ...
          filters.map((ele,ind)=>{
            return(
              <div className='w-full mt-12' key={ind}>
                <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{ele.name}</label>
                <div className='flex'>
                  {ele.content.map((element,index)=>{
                    return(
                      <div key={index} className="relative mx-2">
                        <button className='rounded-[15px] bg-gray-100 hover:bg-gray-300 py-2 px-3'>{element}</button>
                  ...
                  <div className='flex'>
                    <div className='relative ml-2 '>
                      <input type="text" className="pr-3 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Add value" />
        ...
        <button type="button" className="justify-center min-w-[300px] bg-gray-200 hover:bg-[#A042E1] hover:text-white text-gray-600 font-comfortaa inline-flex font-semibold py-2 px-4 rounded-lg shadow">Save and List</button>
      </div>
    </div>
  );
};
